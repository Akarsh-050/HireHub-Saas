...
export const createJob = TryCatch(async (req: AuthenticatedRequest, res) => {
  ...
  const [newJob] =
    await sql`INSERT INTO jobs (title, description, salary, location, role, job_type, work_location, company_id, posted_by_recuriter_id, openings) VALUES 
...
export const updateApplication = TryCatch(
  async (req: AuthenticatedRequest, res) => {
    ...
    const message = {
      ...
      subject: "Application Update - HireHub Saas portal",
